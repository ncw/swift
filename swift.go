...
func (c *Connection) Authenticate() (err error) {
	...
	c.cdnManagementUrl = resp.Header.Get("X-CDN-Management-Url")
	if !c.Authenticated() {
...
// manage is similar to the storage method, but uses the CDN Management URL for CDN specific calls.
func (c *Connection) manage(p requestOpts) (resp *http.Response, headers Headers, err error) {
	return c.call(c.cdnManagementUrl, p)
}
...
func (c *Connection) ContainerCDNEnable(container string, ttl int) (Headers, error) {
	h := Headers{"X-CDN-Enabled": "true"}
	if ttl > 0 {
...
func (c *Connection) ContainerCDNDisable(container string) error {
	h := Headers{"X-CDN-Enabled": "false"}

	_, _, err := c.manage(requestOpts{
		container:  container,
		operation:  "PUT",
		errorMap:   containerErrorMap,
		noResponse: true,
